...
# Fixe for https://github.com/redhat-performance/satellite-performance/issues/61
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no root@"${SATELLITE_SERVER_HOSTNAME}" hammer -u admin -p changeme settings set --name remote_execution_connect_by_ip --value true 
...
# Currently we are using just one DOCKER_HOST_COUNT
